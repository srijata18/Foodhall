package com.example.foodhall.CustomViews
...
import com.example.CustomViews.CustomDurationScroller
...
class AutoScrollViewPager : ViewPager {
    ...
    private var handler_: Handler? = null
    ...
    fun stopAutoScroll() {
        isAutoScroll = false
        handler_!!.removeMessages(SCROLL_WHAT)
    }
    ...
    private fun sendScrollMessage(delayTimeInMills: Long) {
        ...
        handler_!!.removeMessages(SCROLL_WHAT)
        handler_!!.sendEmptyMessageDelayed(SCROLL_WHAT, delayTimeInMills)
    }
    ...
    fun scrollOnce() {
        ...
        if (nextItem < 0) {
            if (isCycle) {
                totalCount?.let { setCurrentItem(it - 1, isBorderAnimation) }
            }
